...
export class LazyForDirective implements OnInit, DoCheck, AfterViewChecked {
    ...
    ngOnInit(): void {
        ...
        let sampleItemElem: HTMLElement = <HTMLElement>{};
        if (this.itemHeight === undefined || this.itemTagName === undefined) {
            this.vcr.createEmbeddedView(this.tpl, {
                $implicit: this.list[0],
                index: 0
            });
            sampleItemElem = <HTMLElement>this.templateElem?.previousSibling;
        }
    ...
    ngAfterViewChecked(): void {
        ...
        this.lazyForContainer = document.body;
    ...
    private update(): void {
        ...
        if (this.list.length === 0) {
            this.vcr.clear();
            if (!this.firstUpdate && this.beforeListElem && this.afterListElem) {
        ...
        if (!this.beforeListElem || !this.afterListElem || !this.lazyForContainer) {
            return;
        }

        if (this.itemHeight === undefined) {
            this.itemHeight = this.templateElem?.parentElement?.firstElementChild?.clientHeight || 0;
        }
        ...
        const fixedHeaderHeight =
            this.beforeListElem.getBoundingClientRect().top -
            this.beforeListElem.scrollTop -
