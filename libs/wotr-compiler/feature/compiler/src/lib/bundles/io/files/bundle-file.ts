...
import { BinaryReader } from '../../binary/binary-reader-temp';
...
export class BundleFile {
    ...
    private async readBlocksAndDirectory(reader: BinaryReader, blocksStream: Stream) {
        for (const blockInfo of this.blocksInfo) {
            ...
            this.directoryInfo = new Node[nodesCount]();
    ...
    private async readFiles(blocksStream: Stream, path: string) {
        ...
        for (let i = 0; i < this.directoryInfo.length; i++) {
            ...
            await file.stream.write(await blocksStream.read(Number(node.size)));
            //await pipeline(blocksStream.asNodeStream(), file.stream.asNodeStream());
    ...
    private async readBlocksInfoAndDirectory(reader: BinaryReader) {
        ...
        switch (
            ...
            case 3: {
                ...
                //const output = decode(Buffer.from(blocksInfoBytes));
        ...
        for (const blockInfo of this.blocksInfo) {
            switch (
                ...
                case 3: {
                    ...
                    const gg = decodeBlock(Buffer.from(compressedBytes), uncompressedBytes);

                    if (gg != uncompressedSize) {
                        throw new Error(
                            `Lz4 decompression error, wrote ${blocksStream.length} bytes but expected ${uncompressedSize} bytes.`
                        );
                    }
