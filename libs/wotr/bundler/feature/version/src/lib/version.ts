...
export class Version {
    public async execute({ gameDirectory, infoFilePath, outputFilePaths }: Options) {
        const versionInfo = await this.get(gameDirectory, infoFilePath);

        const updates = await Promise.all(outputFilePaths.map(filePath => this.update(filePath, versionInfo)));

        return updates.some(update => update === true);
    }
    ...
    private async update(output: string, versionInfo: VersionInfo) {
        ...
        if (!this.isOutOfDate(environment, versionInfo)) {
            console.log(`Version (${output}) is up to date. Skipping...`);
            return false;
        }
