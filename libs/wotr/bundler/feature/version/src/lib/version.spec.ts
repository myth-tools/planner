...
describe('Version', () => {
    ...
    it('should get version info', async () => {
        ...
        const outputFilePaths = ['environment.ts'];
        ...
        await version.execute({ gameDirectory, infoFilePath, outputFilePaths });

        expect(readFileMock).toHaveBeenCalledTimes(2);
        expect(readFileMock.mock.calls[0]).toEqual([join(gameDirectory, infoFilePath), 'utf-8']);
        expect(readFileMock.mock.calls[1]).toEqual([outputFilePaths[0], 'utf-8']);
    ...
    it('should skip update if up to date', async () => {
        ...
        const outOfDate = await version.execute({ gameDirectory, infoFilePath, outputFilePaths });
